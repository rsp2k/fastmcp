    ...
    def validate(self, value: Any) -> str:
        ...
        if self.min_length is not None and len(value) < self.min_length:
            raise ValidationError(
                f"{self.title} must be at least {self.min_length} characters"
            )

        if self.max_length is not None and len(value) > self.max_length:
            raise ValidationError(
                f"{self.title} must be at most {self.max_length} characters"
            )
        ...
        if self.choices and value not in self.choices:
            raise ValidationError(
                f"{self.title} must be one of: {', '.join(self.choices)}"
            )
...
class EnumField(BaseField):
    ...
    def to_schema_property(self) -> dict[str, Any]:
        ...
        if self.choices:
            ...
            schema["enumNames"] = [
                choice.name.replace("_", " ").title() for choice in self.choices
            ]
        else:
            schema["type"] = "string"
    ...
    def validate(self, value: Any) -> str:
        ...
        if self.choices:
            ...
            if value not in valid_values:
                ...
                raise ValidationError(
                    f"{self.title} must be one of: {', '.join(valid_names)}"
                )
